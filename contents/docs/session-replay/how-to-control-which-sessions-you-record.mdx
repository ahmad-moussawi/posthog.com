...
The sampling rate is shared across all platforms. When you configure it on the **Web** tab, the same rate applies to both web and mobile session recordings. Mobile SDK users can view the current rate on the Mobile tab but must use the Web tab to change it.
